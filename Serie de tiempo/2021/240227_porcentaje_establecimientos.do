...
FALTA CAMBIAR RESULTADOS PARA 2023, ESTÁN LOS DEL 2022
...
**Cálculo de porcentaje de matrícula en análisis	
	...
	*bys rbd: replace establecimiento_m=establecimiento_m[_N]
	*bys rbd: keep if _n==1
	tab establecimiento_m //68,19% de la matrícula a nivel nacional
	
	tabstat establecimiento_m, s(mean) f(%9.4f)

********************************************************************************
	
	**Ver porcentaje de EE que no están incorporados en el análisis según zona, código de enseñanza, etc.
	...
	**Variables de Interés 
	...
	bys rbd: keep if _n==1
	tab establecimiento // 28,95% de los establecimientos son rurales
    restore 
	...
	bys rbd: keep if _n==1
	tab establecimiento // 4,49% de los establecimientos son educación parvularia urbanos
    restore 	
	...
	bys rbd: replace establecimiento=0 if establecimiento==2
	bys rbd: keep if _n==1
	tab establecimiento // 0,10% de los establecimientos son educación parvularia rural
	restore
	...
	bys rbd: replace establecimiento=0 if establecimiento==2
	bys rbd: keep if _n==1
	tab establecimiento // 3,39% de los establecimientos son educación adultos urbanos
	restore		
	...
	bys rbd: replace establecimiento=0 if establecimiento==2
	bys rbd: keep if _n==1
	tab establecimiento // 15,50% de los establecimientos son educación especial urbanos
	restore	
