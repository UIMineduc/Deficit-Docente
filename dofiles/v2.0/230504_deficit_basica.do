*Autor: Alonso Arraño
*Fecha ultima modificacion: 05-05-23
...
**# Idoneidad Docente
	...
	*titulo en pedagogia y hacer clases en básica
	...
	gen ido_bas2=.
		replace ido_bas2=0 if nivel2==2 & inlist(subsector2,11001,11004,12001,12002,13001,13002,13003,13004,19001)
		replace ido_bas2=1 if titulo_pedagogia==1 & nivel2==2 & inlist(subsector2,11001,11004,12001,12002,13001,13002,13003,13004,19001)
...
/* OJO PIOJOOOO , NOTA: ESTE CODIGO PODRIA SERVIR EN ALGUN MOMENTO, NO BORRAR!!!!
	*1-NO NETEO! Regional
	...
	collapse (sum) def_total2 if d_def_tot2==1 , by(cod_reg_rbd)
				export excel using "$output\230505_n_def_doc_reg_2022_v2", sheet(basica,modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido2 if d_def_ido2==1 , by(cod_reg_rbd)
				export excel using "$output\230505_n_def_doc_reg_2022_v2", sheet(basica,modify) firstrow(var) cell(E2)
	restore 
	
	*2 - Neteo Regional
		preserve	
	collapse (sum) def_total2  , by(cod_reg_rbd)
				export excel using "$output\230505_n_def_doc_reg_2022_v2", sheet(basica_neto,modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido2  , by(cod_reg_rbd)
				export excel using "$output\230505_n_def_doc_reg_2022_v2", sheet(basica_neto,modify) firstrow(var) cell(E2)
	restore 
...
/* CODIGOS DE ESCENARIOS, NO BORRAR
	...
	*2 - Neteo Comunal
	...
	collapse (sum) def_total2 (first) cod_reg_rbd , by(cod_com_rbd)
				export excel using "$output\230505_n_def_doc_reg_2022_v2", sheet(basica_neto_com,modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido2 (first) cod_reg_rbd  , by(cod_com_rbd)
				export excel using "$output\230505_n_def_doc_reg_2022_v2", sheet(basica_neto_com,modify) firstrow(var) cell(E2)
	restore 
*/
