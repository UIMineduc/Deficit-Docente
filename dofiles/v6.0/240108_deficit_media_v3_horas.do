...
global Output "$main/Output\2022\Nucleares\Horas Media"
...
********************************************************************************
    ...
    **Ciencias = 319628.40625
	...
	tab aux asignatura
    save "$Data\ofta_hrs_rbd_nivel_2022.dta",replace
...
**# Merge Oferta y Demanda de EE
    ...
    use "$Data\ofta_hrs_rbd_nivel_2022.dta",clear
	merge m:1 rbd cod_ense2 using `dda', keepusing( n_cursos dda_hrs_*)
	drop if _merge==1 //79 RBD con docentes pero sin matricula
	drop if _merge==2 //28 RBD con matrícula pero sin docente
	drop _merge
	drop dda_hrs_basica
...
foreach var in leng mat cs hist{
	...
	title("Densidad de la dif. estimada de horas en `z'", color(black) margin(medium)) ///
	legend(label(1 "Horas Totales") label(2 "Horas Idoneo") region(fcolor(none) lcolor(none))) ///
	xtitle("Diferencia horas estimada") ///
	ytitle("Densidad") ///
	graphregion(c(white))
	graph export "$Output\230530_def_doc_`var'_2022_distr_v1_horas.png", replace
	local j = `j' +1
	}
...
**# Figuras finales box plot media
	...
	**Figura Final
	...
	graph export "$Output\230904_def_doc_media_2022_boxplot_v1_horas.png", replace
	...
	graph box def_leng2 def_mat2 def_cs2 def_hist2  , ///
	title("Distribución dif. estimada de horas en enseñanza media", color(black) margin(l-7)) ///
	...
	graph export "$Output\230530_def_doc_media_2022_boxplot_v1_horas.png",replace
	...
	**title("Distribución diferencia estimada de horas en Ens. Media") subtitle("por asignatura") ///
...
**# Base final
	save "$Data\230530_ofta_dda_media_2022_v1_horas.dta",replace
	
	
	
****************************** Resultados finales ******************************	
	
	use "$Data\230530_ofta_dda_media_2022_v1_horas",clear
...
**# Porcentaje de EE con déficit por región y asignatura
	...
	*table  cod_reg_rbd d_def_`var'2 
	...
	export excel using "$Output\231019_porcent_def_media_reg_2022_v1_horas.xlsx", sheet(EE_def_region,modify) firstrow(var) cell("`letter'2")
	restore
	local x=`x'+5
	}
...
**# Total de docentes que faltan por región y asignatura
	...
	export excel using "$Output\231019_def_media_reg_2022_v1_horas.xlsx", sheet(reg_`var',modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido_`var'2 if d_def_ido_`var'2==1 , by(cod_reg_rbd)
	export excel using "$Output\231019_def_media_reg_2022_v1_horas.xlsx", sheet(reg_`var',modify) firstrow(var) cell(G2)
	restore 
	
	preserve	
	collapse (sum)  dda_hrs_`var', by(cod_reg_rbd)
	export excel using "$Output\231019_def_media_reg_2022_v1_horas.xlsx", sheet(reg_`var',modify) firstrow(var) cell(K2)
	restore 	
	}	
...
**# Total de docentes que faltan por dependencia y asignatura
	...
	**Definición de Dependencia
	...
	sort depe
	export excel using "$Output\231019_def_media_depe_2022_v1_horas.xlsx", sheet(depe_`var',modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido_`var'2 if d_def_ido_`var'2==1 , by(depe)
	sort depe
	export excel using "$Output\231019_def_media_depe_2022_v1_horas.xlsx", sheet(depe_`var',modify) firstrow(var) cell(F2)
	restore 
	}	 
...
**# Porcentaje de EE con déficit por dependencia y asignatura
	...
	export excel using "$Output\231019_porcent_def_media_depe_2022_v1_horas.xlsx", sheet(EE_def_depe,modify) firstrow(var) cell("`letter'2")
	restore
	local x=`x'+5
	}	
...
**# Porcentaje de EE con déficit por SLEP	
	...
	**Variable dicotómica para cada SLEP y asignatura
	...
	export excel using "$Output\231019_porcent_def_media_depe_2022_v1_horas.xlsx", sheet(EE_def_depe_`var',modify) firstrow(var) cell(B2)
	restore
	}