...
**# Horas totales del establecimiento*
	...
	*ajustes del merge
	...
	gen dotacion_docente=doc_ifp+doc_ifs
	drop ifp ifs
...
**# Horas totales y Horas Lectivas
	...
	*Generamos Asignaturas 
	...
	collapse (sum) hrs_lect2 hrs_lect_ido2 (first) doc_ifp doc_ifs dotacion_docente, by(rbd asignatura)
	
	rename ( hrs_lect2 hrs_lect_ido2) ( ofta_hrs2 ofta_hrs_ido2 )

	*Nota: Tenemos 2.672  rbd -> pasa a 2.647 después de borrar los asignatura==.
...
**# Merge demanda y oferta + info administrativa
	...
	drop if _merge==2 //172 no hacen merge
	drop _merge
	drop dda_hrs_basica
...
**# Cálculo del deficit
	...
	*Deficit general e Idoneo - 1+2
	...
	foreach var in leng mat cs hist{
		gen def_`var'2=round(ofta_hrs2 - dda_hrs_`var',2) if asignatura==`i'
					replace def_`var'2=round((def_`var'2)/(30*0.65),2)
		gen def_ido_`var'2=round(ofta_hrs_ido2- dda_hrs_`var',2) if asignatura==`i'
					replace def_ido_`var'2=round((def_ido_`var'2)/(30*0.65),2)
	local i=`i'+1
	}
...
foreach var in leng mat cs hist{
	...
	ytitle("Densidad") ///
	graphregion(c(white))
	graph export "$output\230526_def_doc_`var'_2022_distr.png", replace
	local j = `j' +1
	}
...
**# Cálculo del Deficit
	...
	*horas 1+2
	...
	save "230526_ofta_dda_media_2022.dta",replace
...
local i=2
	...
	export excel using "$output\230526_n_doc_media_reg_2022_v38s.xlsx", sheet(EE_def,modify) firstrow(var) cell("`letter'2")
	restore
	local i=`i'+5
	}
...
******Total de docentes que faltan por comuna,región y asignatura
	...
	*1-NO NETEO! Comunal
	...
	collapse (sum) def_`var'2 (first) cod_reg_rbd if d_def_`var'2==1 , by(cod_com_rbd)
				export excel using "$output\230526_n_doc_media_reg_22_`var'_38s.xlsx", sheet(media_com,modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido_`var'2 (first) cod_reg_rbd if d_def_ido_`var'2==1 , by(cod_com_rbd)
				export excel using "$output\230526_n_doc_media_reg_22_`var'_38s.xlsx", sheet(media_com,modify) firstrow(var) cell(F2)
...
**# Analisis - Dependencia
	...
	*use "230526_ofta_dda_media_2022",clear
...
*/
	...
	*NO NETEO! Comunal
	...
	export excel using "$output\230526_def_media_depe.xlsx", sheet("depe_`var'",modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido_`var'2 (first) cod_reg_rbd if d_def_ido_`var'2==1 , by(cod_com_rbd depe)
	sort depe cod_com_rbd
	export excel using "$output\230526_def_media_depe.xlsx", sheet("depe_`var'",modify) firstrow(var) cell(F2)
	restore 
	}
