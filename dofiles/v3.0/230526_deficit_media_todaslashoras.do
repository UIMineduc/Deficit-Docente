...
**# Horas totales del establecimiento*
	...
	*Agregamos la información de la cantidad de horas para cada oferta por rbd
	...
	merge 1:1 rbd sector1 subsector1 using `ofta2', nogen 
	codebook rbd
...
**# Merge demanda y oferta + info administrativa
	...
	drop if _merge==2 // 171 (150) no hacen merge
	drop _merge
	drop dda_hrs_basica
	...
	*Deficit general total
	...
	foreach var in leng mat cs hist{
		gen def_`var'2=ofta_hrs2 - dda_hrs_`var' if asignatura==`i'
					replace def_`var'2=floor((def_`var'2)/(30*0.65)) if def_`var'2<0
					replace def_`var'2=ceil((def_`var'2)/(30*0.65)) if def_`var'2>0
	local i=`i'+1
	}
	...
	*use "ofta_dda_media_2022",clear
	...
	foreach var in leng mat cs hist {
		...
		tabstat d_def_`var'2, by(cod_reg_rbd) s(mean) f( %9.3f)
			table d_def_`var'2 cod_reg_rbd
	}
...
local i=2
	...
	export excel using "$output\230526_n_doc_media_reg_2022_v38s", sheet(EE_def,modify) firstrow(var) cell("`letter'2")
	restore
	local i=`i'+5
	}
...
**# Total de docentes que faltan por comuna,región y asignatura
	...
	*1-NO NETEO! Comunal
	...
	display "exportando la asignatura `var' "
				export excel using "$output\230526_n_doc_media_reg_22_`var'_38s", sheet(media_com,modify) firstrow(var) cell(B2)
	restore 
}	
