...
**# 1 - Cantidad de cursos distintos por nivel (básica y media)
	...
	* total de cursos por EE
	collapse (sum) n_cursos (mean) tam_curso (first)  jornada id_grado cod_reg_rbd cod_com_rbd, by(rbd cod_ense2  cod_grado)
...
merge m:1 id_grado using "plan_de_estudios_2022.dta", keepusing($ramos_ee) keep(3)

	foreach var of varlist $ramos_ee{
	gen dda_hrs_`var'=`var'*n_cursos
}

collapse (sum)  dda_hrs_basica dda_hrs_hist dda_hrs_cs dda_hrs_leng dda_hrs_mate  (first) n_cursos, by(rbd cod_ense2)


save "dda_hrs_rbd_nivel_2022.dta",replace
