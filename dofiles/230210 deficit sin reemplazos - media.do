...
local i=2
	...
	export excel using "$output\230210_noreemplazo.xlsx", sheet(media_ee,modify) firstrow(var) cell("`letter'2")
	restore
	local i=`i'+5
	}
...
******Total de docentes que faltan por región y asignatura
	*1-NO NETEO! Regional
	...
	export excel using "$output\230209_sim_leng_noreemplazo", sheet(media,modify) firstrow(var) cell(A2)
	restore 

	preserve	
	
	collapse (sum) def_ido_`var'2 if d_def_ido_`var'2==1 , by(cod_reg_rbd)
	export excel using "$output\230209_sim_leng_noreemplazo", sheet(media,modify) firstrow(var) cell(E2)
	...
	*2 - Neteo Regional
	...
	export excel using "$output\230209_sim_leng_noreemplazo", sheet(media_neto,modify) firstrow(var) cell(A2)
				
	restore 

	preserve	
	collapse (sum) def_ido_`var'2  , by(cod_reg_rbd)
				export excel using "$output\230209_sim_leng_noreemplazo", sheet(media_neto,modify) firstrow(var) cell(E2)
	restore 
...
******Total de docentes que faltan por comuna,región y asignatura
	...
	*1-NO NETEO! Comunal
	...
	collapse (sum) def_`var'2 (first) cod_reg_rbd if d_def_`var'2==1 , by(cod_com_rbd)
				export excel using "$output\230209_sim_leng_noreemplazo", sheet(media_com,modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido_`var'2 (first) cod_reg_rbd if d_def_ido_`var'2==1 , by(cod_com_rbd)
				export excel using "$output\230209_sim_leng_noreemplazo", sheet(media_com,modify) firstrow(var) cell(F2)
	restore 
	
	*2 - Neteo Comunal
		preserve	
	collapse (sum) def_`var'2 (first) cod_reg_rbd , by(cod_com_rbd)
				export excel using "$output\230209_sim_leng_noreemplazo", sheet(media_neto_com,modify) firstrow(var) cell(B2)
	restore 

	preserve	
	collapse (sum) def_ido_`var'2 (first) cod_reg_rbd  , by(cod_com_rbd)
				export excel using "$output\230209_sim_leng_noreemplazo", sheet(media_neto_com,modify) firstrow(var) cell(G2)
	restore 
	}	
